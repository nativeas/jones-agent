...
async def _run() -> None:
    ...
    try:
        ...
        # C (#8 #9): same kind of stand-in, with `.config` also populated —
        # `DaemonContext.config` is C's field per §1, and needs to already be
        # reachable here for W3's rule gate / A's session code to pick up once they
        # land, even though nothing in this branch's own register() calls reads it
        # yet (see docs/design/01-w2-interfaces.md §1, §4).
        ctx = types.SimpleNamespace(db=conn, config=DefaultConfigResolver(conn))
