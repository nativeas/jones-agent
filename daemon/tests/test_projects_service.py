...
def test_ensure_default_project_corrects_the_placeholder_path_to_the_real_home(conn, tmp_path):
    # 004 seeds proj_default with the placeholder path '__pending_home__' (a static
    # SQL migration can't know the per-machine home dir) — this is the runtime fix.
    service = ProjectService(conn)
    assert service.get(DEFAULT_PROJECT_ID)["path"] == "__pending_home__"
