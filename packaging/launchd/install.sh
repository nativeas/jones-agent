#!/usr/bin/env bash
# 草案：把 com.jones.daemon.plist 安装为用户级 LaunchAgent（第二轮评审新增，见修复记录）。
# 跟 plist 本身一样是草案——真实的"首次启动时安装"逻辑最终要写进 apps/desktop/ 的 Electron
# main 进程启动代码里（daemon 可执行文件路径届时是已知的），这里只提供可读的参考实现 +
...
DEST_PLIST="$LAUNCH_AGENTS_DIR/com.jones.daemon.plist"

sed \
  -e "s#__DAEMON_EXECUTABLE__#${DAEMON_EXECUTABLE}#g" \
  -e "s#__JONES_HOME__#${JONES_HOME}#g" \
  com.jones.daemon.plist > "$DEST_PLIST"
