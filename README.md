...
**v1.0（macOS）实施中**，P0 全部完成，测试 1038 pytest + 129 vitest 全绿。
...
- **#36** `cryptography` 无 macOS x86_64 wheel，arm64 机器出不了 Intel 包（挡 G18 双架构）——已定：CI 加 Intel runner 源码编译
- **#33** 真实 Apple 公证（需开发者证书）
- **#35** daemon 退出阶段偶发挂起——已定：带着发，写进 release notes
- **#40** 两条真实模型 E2E 连续跑时互相干扰（CI 不受影响）
