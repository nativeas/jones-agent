...
- **`_terminate_run` 的幂等保护（round-N2 controller 裁定 R-N1 收紧）**：写库前先读一次 `runs.status`，非 `'running'` 时不再无条件跳过——`_on_worker_crash` 的 N07 兜底和"真的 `prompt()` 自己发现"两条路径理论上可能在极窄的窗口内都想终止同一个 Run，其中一条可能在 `mark_run_terminated` 的 DB 写已经落地、但自己的 `run.terminated` 广播还没发出去之前被 `task.cancel()` 打断（`store/db.py` 单线程 `ThreadPoolExecutor`，取消 asyncio 层的 await 不会停掉已经在 DB 线程上跑着的 UPDATE）。`SessionService` 因此额外维护一个内存 `set[run_id]`，只记录"广播过"（不是"写过"）——幂等守卫读到非 `running` 时，先查这个 run_id 是否已经广播过：广播过，才是真无操作；没广播过，就从持久化的 `runs` 行重建 `ErrorCard` 并补发一次 `run.terminated`，保证每个终止的 Run 至少广播一次，而不是把"双写/双广播"当成需要一律压掉的问题。
