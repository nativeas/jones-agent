...
- **11.2 的 Step 数 / 时长上限触发预算终止**：需要在 `_run_turn`/`_handle_tool_call_start` 里主动数 Step、算耗时并在触顶时调用 `_terminate_run(kind="budget", ...)`——这两个函数是 A/#10 的专属函数，04-w5-interfaces.md §1 没有把它们开放给 N；本分支只让 `_terminate_run` 在**收到** `kind="budget"` 时能正确出卡，没有新增任何调用点去触发它。同理 token 预算（Goal 层面）依赖 FR17（P1，未实现）。round-2 评审裁定：这不能停在"分支报告的没做什么"里——按 `05-w6-interfaces.md` §0 的规则本应开一个 `[验收缺口]` Issue，但本轮修复执行时 `gh issue create` 被 Claude Code 的 auto-mode 分类器拦截（副作用外部操作，未经人工批准）；命令与文案已写进本分支报告"round-2 修复记录"的 open 列表，留给控制者/用户手动执行或裁定归属分支。
...
- **`_advance_queue` 对错误终止照样立即续跑，PRD 9.3"放弃"清的已经不是队列里的下一条（round-2 发现，未修复，待裁定）**：`_run_turn` 的 `finally: await self._advance_queue(session_id)` 无差别调用——不管这个 Run 是成功、用户停止还是错误/预算终止，队列里排队的下一条指令立刻 `pop_next_queue_item` + `_start_turn`，早于错误卡片广播后用户能点"放弃"的时间点。`_advance_queue` 不在 N/#22 的授权改动点（`sessions/service.py` 只准改 `_terminate_run`/`_on_worker_crash`/新增 `retry`），本分支不能单方面改它，也不能单方面改 PRD 9.3"终止不清空 Session 队列；队列中的后续指令挂起，等用户决定继续或清空"这句话。留给控制者裁定：(a) `_advance_queue` 对 `kind != "user"` 的终止不自动续跑（A/#10 territory），或 (b) PRD 9.3 这句话改掉，接受"错误/预算终止后队列仍自动续跑，放弃只能清掉更后面的"。裁定之前，"放弃"的实际效果（清掉的是"当时还没出队的那些"）与它的措辞（"清掉排队中的后续"）不完全一致——不是本分支引入的新问题，是 round-2 评审在既有行为上发现的既有落差。
